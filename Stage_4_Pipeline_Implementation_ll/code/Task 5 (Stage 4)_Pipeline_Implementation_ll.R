...
library(pROC)
library(iml)
library(factoextra)  # Added this line to ensure fviz_cluster works
set.seed(1234)
...
# Select top 3000 genes with highest standard deviation
topPredicts <- order(SDs, decreasing = TRUE)[1:3000]

# Reduce glioma.data to only include top 3000 genes
glioma.data <- glioma.data[, topPredicts]
...
# Merge scaled data with metadata
merge.data <- merge(scaled_data_df, glioma.meta, by = "row.names")
rownames(merge.data) <- merge.data$Row.names
merge.data$Row.names <- NULL  # Remove the 'Row.names' column after setting it as row names
...
fviz_cluster(kmeans_result, 
             ...
             pointsize = 2,        # Adjust point size for better visibility
             ggtheme = theme_minimal()) +
  # Add colors based on IDH status from the merge.data
  geom_point(data = merge.data, aes(color = IDH.status)) +  # Corrected to use merge.data
  labs(color = "IDH Status") +  # Label for the legend
  scale_color_manual(values = c("Mutant" = "blue", "WT" = "red"))
